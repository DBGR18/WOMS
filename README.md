...
- Sales users can add customer orders to pending scheduling only; customer order due dates must be tomorrow or later in the selected production line's timezone, and invalid due dates show `無法被接受的交期`. Draft feasibility is checked against existing scheduled allocations plus the current pending backlog for that line; the preview dialog shows those pending allocations and waterline usage, and it flags pending orders that the new draft would push past their due date. The main monthly calendar remains unchanged. Scheduler engineers' formal scheduling flow still considers only the selected pending orders. Order notes are write-on-create only; rejected-order resubmission can adjust due date and quantity but cannot rewrite the original note.
